...
%% at max force.
%function [x, RESNORM, RESIDUAL, EXITFLAG] = Step_ABC_Time_Fit_v4(t, Force, x)

function [best_p, StepForce_Fit, exit_flag] = Step_ABC_Time_Fit_v4(varargin)
...
%                                   A  k   B b C c
% Prehaps better to use this scaling vector if fitting on Modulus(t) values.
%addOptional(params,'scaling_vector',[100 0.1 100 1 100 10]);
% Prehaps better to use this scaling vector if fitting on Stress(t) values.
addOptional(params,'scaling_vector',[1 0.1 1 1 1 10]);
...
%                                   A  k   B b C c
addOptional(params,'upper_bounds',[inf 1 inf inf inf inf]);
