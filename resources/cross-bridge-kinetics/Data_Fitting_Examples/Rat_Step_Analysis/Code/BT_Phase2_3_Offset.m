...
%% at max force.
%function [x, RESNORM, RESIDUAL, EXITFLAG] = A_Proc_Phase2_3_Offset(t, Force, x)

function [best_p, StepForce_Fit, exit_flag] = BT_Phase2_3_Offset(varargin)

% Handle inputs
params = inputParser;
addRequired(params,'time');
addRequired(params,'force');
addOptional(params,'scaling_vector',[1 1 1 1 1]);
addOptional(params,'initial_p',[1 1 1 1 1]);
addOptional(params,'lower_bounds',[zeros(1,4), -1e3]);
addOptional(params,'upper_bounds', [1e3*ones(1,5)]);
