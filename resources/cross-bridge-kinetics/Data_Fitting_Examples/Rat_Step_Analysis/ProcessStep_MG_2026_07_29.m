...
StepBegin = 0; %[seconds] when begin estimating Step and Step Response
StepEnd = 0.5; %[seconds] when end estimating Step and Step Response
...
fn = 'Test_Step_Analysis_List_MG_2025_09_19.xlsx';
...
for file_num=1:length(all_data.Fiber_Num)
    ...
    animal=all_data.Animal_ID(file_num);     %get cell data
    ...
    if (strcmp(protocol, 'Step') & (trial_num == 1 || trial_num==2) )% ...
        ...
        set(figure(1), 'Position', [0, 0, pix_per_inch*6, pix_per_inch*6])
        ...
        t_3_estimate = 0.015; % 15 milliseconds search range following step?
        ...
        % Find t34
        % (time to get half way decayed from Stress_t3 to Stress_t4)
        [value, index_t34] = min(abs(Step_Stress(index_t3:index_t4)-mean([Stress_t3, Stress_t4])));
        index_t34 = (index_t3-1)+index_t34; %% Need to offset 1 with min(abs...) just above
        Stress_t34=Step_Stress(index_t34, 1);
        t34 = Step_time(index_t34, 1);
        ...
        plot(t1, Stress_t1, 'ko')
        plot(t12, Stress_t12, 'ro')
        plot(t2, Stress_t2, 'co')
        plot(t23, Stress_t23, 'r*')
        plot(t3, Stress_t3, 'mo')
        plot(t34, Stress_t34, 'ro')
        ...
        %plot(Step_time(index_t0:end), (Step_Stress(index_t0:end)./Step_Strain(index_t0:end))/max(Step_Stress(index_t0:end)./Step_Strain(index_t0:end)), 'k-'), hold on
        %plot(Step_time(index_t0:end), Step_Stress(index_t0:end)/Stress_t1, 'b-')
        ...
        plot(t_Fit+t1, A2*(exp(-kf_12*t_Fit)), 'k-')
        plot(t_Fit+t1, A3*(1-exp(-kf_23*t_Fit)), 'r-')
        plot(t_Fit+t1, A3*(1-exp(-kf_df*t_Fit)), 'r--')
        plot(t_Fit+t1, A3*(exp(-kf_34*t_Fit)), 'm-')
        plot(t0, Stress_t0, 'go')
        plot(t1, Stress_t1, 'ko')
        plot(t12, Stress_t12, 'ko')
        plot(t2, Stress_t2, 'ro')
        plot(t23, Stress_t23, 'ro')
        plot(t3, Stress_t3, 'mo')
        plot(t34, Stress_t34, 'mo')
        plot(t4, Stress_t4, 'ko')
        ylabel('Step Stress (mN mm-2)')
        xlabel('Time (seconds)')
        xlim([0, StepEnd])
        legend('Stress(t1-end)', 'Exp Decay kf_{12}', 'Exp Rise kf_{23}', ...
            'Exp Rise kf_{df}', 'Exp Decay kf_{34}', 'location', 'northeast')
        grid
        title(['Isometric Force = ' num2str(InitialIsometricForce) 'mN; Isometric Stress = ' num2str(InitialIsometricStress) ' mN/mm2'])
        xlim([0, StepEnd])
        ...
        title(['Total=3 Exp Fit; and 3 Piecewise 1 Exp Fits'])
        xlim([0, StepEnd])
        ...
        T_Fit=Step_Stress(index_t1:index_t3, 1); % Use only data from t1 onward
        t_Fit=Step_time(index_t1:index_t3, 1); % Set fit zero time at t0
        %t_Fit=Step_time(index_t1:index_t3, 1)-t1; % Set fit zero time at t1
        ...
        %[x_23, fit_23, exitflag_1] = BT_Phase2_3_Fit(t_Fit, T_Fit, 'initial_p', ParamGuess');
        [x_23, fit_23, exitflag_1] = BT_Phase2_3_Offset(t_Fit, T_Fit, 'initial_p', [ParamGuess; 1]');
        R = corrcoef(T_Fit, fit_23);
        R2 = R(1,2)*R(1,2);
        %t_plot_23=t_Fit;
...
%        plot(t_plot_34, fit_34, 'm-')
        text(t2, Stress_t1*0.9, ['T_{23}fit=' num2str(x_23(1,1), '%.1f') 'exp(-' num2str(x_23(1,2), '%.2f') 't)+' ...
            num2str(x_23(1,3), '%.1f') '(1-exp(-' num2str(x_23(1,4), '%.2f') 't)+' num2str(x_23(1,5), '%.1f')  ',  R2=' num2str(R2, '%.2f')])
        % text(t34, Stress_t1*0.3, ['T_{34}=' num2str(x_34(1,1), '%.1f') 'exp(-' num2str(1/x_34(1,2), '%.2f') 't)+' ...
        %     num2str(x_34(1,3), '%.1f')])
        grid
        title(['2 Exp Fit Stress(t1-t3) and 1 Exp Fit Stress(t3-t4)'])
        xlim([0, StepEnd])
        ...
        if(SarcLength==2.7 & strcmp('Cntrl', all_data.Cond{file_num, 1}) ) % SL = 2.7 and Control Fiber
            ...
            StepData_27.k_t34 = [StepData_27.k_t34; kf_34];
        ...
        elseif(SarcLength==2.4 & strcmp('Cntrl', all_data.Cond{file_num, 1}) ) % SL = 2.4 and Control Fiber
            ...
            StepData_24.k_t34 = [StepData_24.k_t34; kf_34];
...
% % Write Out Fit Step Figure for each Run.
% Out_Fig_Dir = 'J:\TannerBertGroup\Lab_Mechanics\NSF_Skeletal\Acquired_2025\SL_Step_FVel_OM\Step_Notes_BT\MG_Analysis\SummaryPlots\';
% Out_Fig_Name = 'SumPlotStep_OLD_0OM';
